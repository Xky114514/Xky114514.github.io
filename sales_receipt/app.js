(() => {
  ...
  const detailSpecTargets = [
    ...
    },
    {
      ...
      html: specHtml(
        ...
        [
          ...
          "签收数与下单数不一致时输入框标红，并通过提示图标说明系统已自动加入售后处理。",
    ...
    },
    {
      ...
      html: specHtml(
        ...
        ["签收数变化时当前行立即重算；数量不一致的商品自动加入售后处理，并默认选择“商品异常—异常”，售后数量仍由操作员填写。"],
        ["差异只针对当前关联销售订单商品计算。"],
      ),
    },
    {
      ...
      title: "售后处理｜页内集中维护",
      html: specHtml(
        "售后处理常驻展示在商品核对上方，并按商品分组维护。",
        [
          "签收数与下单数不一致时，系统自动加入该商品并默认选择“商品异常—异常”。",
  ...
  function createAutomaticProductAfterSalesAction(itemId) {
    return normalizeProductAfterSalesAction({
      itemId,
      type: "product_exception",
      origin: "current",
  ...
  function renderAfterSalesWorkbenchRow(action, index, sourceRow = null) {
    ...
    const automaticRequired = Boolean(
      action?.autoGenerated && Number.isFinite(difference) && difference !== 0,
    );
  ...
  function renderAfterSalesProductGroup(group, groupIndex) {
    ...
    const hasRequiredAutomaticAction = group.actions.some(
      (action) => action.autoGenerated && Number.isFinite(difference) && difference !== 0,
    );
    ...
    const controls = canAppend
      ? `<button class="btn text" type="button" data-add-after-sales-action data-item-id="${escapeHTML(group.itemId)}">＋ 添加售后类型</button>${hasLockedActions || hasRequiredAutomaticAction ? "" : `<button class="link danger" type="button" data-remove-after-sales-product data-item-id="${escapeHTML(group.itemId)}">移除商品</button>`}`
      : "";
  ...
  function ensureInlineReceiptWorkspace() {
    const card = one(".receipt-ai-panel");
    const afterSalesPanel = one(".receipt-after-sales-panel", card);
    const productsPanel = one("[data-receipt-workspace-panel=\"products\"]", card)
      || one(".receipt-products-panel", card);
    if (!card || !afterSalesPanel || !productsPanel) return;

    one(".receipt-workspace-tabs", card)?.remove();
    productsPanel.classList.add("receipt-products-panel");
    productsPanel.removeAttribute("data-receipt-workspace-panel");
    afterSalesPanel.removeAttribute("data-receipt-workspace-panel");
    afterSalesPanel.hidden = false;

    let afterSalesTitle = one("[data-inline-section-title=\"after-sales\"]", card);
    if (!afterSalesTitle) {
      afterSalesTitle = document.createElement("div");
      afterSalesTitle.className = "receipt-inline-section-title";
      afterSalesTitle.dataset.inlineSectionTitle = "after-sales";
      afterSalesTitle.innerHTML = "<strong>售后处理</strong>";
    }
    let productsTitle = one("[data-inline-section-title=\"products\"]", card);
    if (!productsTitle) {
      productsTitle = document.createElement("div");
      productsTitle.className = "receipt-inline-section-title";
      productsTitle.dataset.inlineSectionTitle = "products";
      productsTitle.innerHTML = "<strong>商品核对</strong>";
    }
    productsPanel.before(productsTitle);
    productsTitle.before(afterSalesPanel);
    afterSalesPanel.before(afterSalesTitle);
  }

  function setReceiptWorkspace(workspace = "products", renderWorkbench = true) {
    ensureInlineReceiptWorkspace();
    const showAfterSales = workspace === "after-sales";
    if (showAfterSales && renderWorkbench) renderAfterSalesWorkbench();
    const target = showAfterSales
      ? one("[data-after-sales-workbench]")
      : one("[data-inline-section-title=\"products\"]") || one("[data-products-table-wrap]");
  ...
  function readAfterSalesWorkbenchRow(row) {
    ...
    return {
      ...
      autoGenerated: row.dataset.actionAutoGenerated === "true",
      submittedAt: row.dataset.actionSubmittedAt || "",
    };
  }

  function syncAutomaticAfterSalesForRows(
    rows = all("[data-order-line]"),
    { render = true, persist = true } = {},
  ) {
    if (document.body.dataset.readonly === "true") return false;
    ensureProductAfterSalesActionsLoaded();
    if (Array.isArray(productAfterSalesDraftActions) && one("[data-after-sales-body]")) {
    ...
    let actions = (
      Array.isArray(productAfterSalesDraftActions)
        ...
        : productAfterSalesActions
    ).map((action) => normalizeProductAfterSalesAction(action));
    let changed = false;
    ...
    rows.forEach((row) => {
      ...
      const ordered = Number(row.dataset.outbound);
      const signed = raw === "" ? Number.NaN : Number(raw);
      const valid = /^\d+(?:\.\d{0,2})?$/.test(raw) && Number.isFinite(signed) && signed >= 0;
      if (!itemId || !Number.isFinite(ordered) || !valid) return;

      const mismatch = signedDifference(ordered, signed) !== 0;
      const itemActions = actions.filter((action) => action.itemId === itemId);
      if (mismatch && itemActions.length === 0) {
        actions.push(createAutomaticProductAfterSalesAction(itemId));
        changed = true;
      } else if (!mismatch && itemActions.some((action) => action.autoGenerated)) {
        actions = actions.filter(
          (action) => action.itemId !== itemId || !action.autoGenerated,
        );
        changed = true;
      }
    });
    ...
    productAfterSalesActions = actions.map((action) => ({ ...action }));
    productAfterSalesDraftActions = actions.map((action) => ({ ...action }));
    syncProductActionIndicators();
    if (persist) persistProductAfterSalesActions();
    if (render) renderAfterSalesWorkbench();
    updateReceiptSummary();
    applyLargeOrderView();
    return true;
  }
  ...
  function openAfterSalesWorkbench(seedActions = null) {
    ...
    ensureProductAfterSalesActionsLoaded();
    if (!Array.isArray(seedActions) && one("[data-after-sales-body]")) {
      snapshotAfterSalesWorkbenchDraft();
    }
  ...
  function initializeAfterSalesData() {
    ...
    ensureProductAfterSalesActionsLoaded();
    if (document.body.dataset.readonly !== "true") {
      syncAutomaticAfterSalesForRows(all("[data-order-line]"), {
        render: false,
        persist: true,
      });
    }
  ...
  function ensureSignedQuantityWarning(row) {
    const wrap = one(".quantity-input-wrap", row);
    if (!wrap) return null;
    let warning = one("[data-signed-quantity-warning]", wrap);
    if (warning) return warning;
    warning = document.createElement("span");
    warning.className = "signed-quantity-warning";
    warning.dataset.signedQuantityWarning = "";
    warning.dataset.tooltip = "签收数与下单数不一致，系统已自动加入售后处理。";
  ...
  function updateSignedQuantityWarning(row, difference) {
    const input = one(".actual-input", row);
    const warning = ensureSignedQuantityWarning(row);
    const mismatch = Number.isFinite(difference) && difference !== 0;
    input?.classList.toggle("signed-quantity-mismatch", mismatch);
    one(".quantity-input-wrap", row)?.classList.toggle(
      "has-signed-quantity-warning",
      mismatch,
    );
  ...
  function updateQuantityRow(input) {
    ...
    if (!valid) {
      ...
      updateSignedQuantityWarning(row, Number.NaN);
      if (variance) {
    ...
    updateSignedQuantityWarning(row, difference);
    if (variance) {
  ...
  function bindAfterSalesEditing() {
    ...
    document.body.addEventListener("change", (event) => {
      ...
      if (select) {
        ...
        select.classList.remove("invalid");
        workbenchRow.dataset.actionAutoGenerated = "false";
    ...
    document.body.addEventListener("input", (event) => {
      ...
      if (actionRow) {
        actionRow.dataset.actionAutoGenerated = "false";
        refreshAfterSalesProductGroupState(
  ...
  function bindQuantityEditing() {
    ...
    inputs.forEach((input) => {
      ...
      input.addEventListener("input", () => {
        ...
        if (row) {
          row.dataset.modified = "true";
          quantityValid = updateQuantityRow(input);
          row.dataset.reviewed = quantityValid ? "true" : "false";
          if (quantityValid) syncAutomaticAfterSalesForRows([row]);
        }
  ...
  function renderOrderLine(line, index, receiptItem = null) {
    ...
    return `
      <tr class="${rowClass}" data-product-row data-quantity-row data-order-line data-original-index="${index}" data-reviewed="${document.body.dataset.readonly === "true" || (Number.isFinite(difference) && Boolean(recognizedName)) ? "true" : "false"}" data-modified="false" data-order-product-name="${escapeHTML(line.name)}" data-recognized-name="${escapeHTML(recognizedName)}" data-ai-text="${escapeHTML(aiText)}" data-outbound="${line.outbound}" data-unit="${escapeHTML(line.unit)}" data-current-diff="${Number.isFinite(difference) ? difference : ""}" data-after-sales-selected="${afterSalesSelected}" data-after-sales-type="${afterSalesType}" data-after-sales-reason="${escapeHTML(afterSalesReason)}" data-after-sales-quantity="${escapeHTML(afterSalesQuantity)}" data-after-sales-quantity-manual="true" data-abnormal-count="${afterSalesType === "product_exception" ? afterSalesQuantity : ""}" data-return-count="${afterSalesType === "product_return" ? afterSalesQuantity : ""}" data-return-inbound-id="${escapeHTML(returnInboundId)}">
        ...
        <td><div class="quantity-input-wrap"><input class="quantity-input actual-input${Number.isFinite(difference) && difference !== 0 ? " signed-quantity-mismatch" : ""}" inputmode="decimal" value="${escapeHTML(actual)}" placeholder="请填写" aria-label="${escapeHTML(line.name)}签收数" data-item-id="${escapeHTML(line.id)}"${receiptItem ? ' data-preserve-current="true"' : ""}><span class="unit-suffix">${escapeHTML(line.unit)}</span><span class="signed-quantity-warning" data-signed-quantity-warning data-tooltip="签收数与下单数不一致，系统已自动加入售后处理。" role="img" tabindex="0" aria-label="签收数与下单数不一致，系统已自动加入售后处理。"${Number.isFinite(difference) && difference !== 0 ? "" : " hidden"}>!</span></div></td>
  ...
  function applyDetailReadOnlyMode() {
    ...
    all(
      ...
      (input) =>
        !supportsAfterSalesContinuation() ||
        !input.closest("[data-after-sales-workbench]"),
    ).forEach((input) => {
      input.readOnly = true;
    ...
    all(".receipt-ai-panel select:not([data-product-sort])").filter(
      (select) =>
        !supportsAfterSalesContinuation() ||
        !select.closest("[data-after-sales-workbench]"),
    ).forEach((select) => {
