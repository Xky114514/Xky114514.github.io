(() => {
  ...
  const orderProfiles = {
    ...
    "prompts.html": profile("提示词", "维护系统基础提示词和客户专属提示词，并按正常启用客户检查配置覆盖情况。", ["客户视图展示全部正常启用客户，未配置客户专属提示词的客户也必须出现；停用客户默认不展示。", "客户视图和详情只统计启用中的客户提示词绑定，停用模板不进入启用绑定数和当前配置。", "列表不展示客户 ID；订单解析和意图识别字段分别最多展示 2 个提示词，超出部分以 +N 表示。", "系统基础配置与客户专属配置分层展示；客户专属为空不代表系统提示词未生效。"], [field("提示词类型", "筛选订单解析或意图识别提示词。", "系统枚举。", "选项固定为全部、订单解析、意图识别。"), field("配置状态", "按客户是否存在启用中的专属提示词绑定筛选。", "启用绑定汇总。", "选项固定为全部、已配置、未配置；停用模板不改变配置状态。"), field("关联群聊", "展示客户当前关联的消息来源群聊。", "客户与群聊关系。", "多群聊在列表显示首个名称和 +N，详情展示完整列表。"), field("系统基础配置", "只展示订单解析、意图识别系统提示词的配置状态。", "系统提示词。", "不展示更新时间；内容为空时显示“系统基础提示词未配置”警告。"), field("客户专属配置", "只展示该客户启用中的模板名称和类型。", "客户提示词启用绑定。", "不展示内容摘要及更新时间；无绑定时说明当前仅使用系统基础配置。")], ["查看：打开右侧详情抽屉，客户信息只展示名称和完整关联群聊。", "查看系统提示词：切换到系统提示词页签。", "管理该客户提示词：切换到客户提示词页签并按客户 ID 筛选，继续使用统一的新建、编辑、启停和绑定流程。", "清除筛选：搜索无结果时恢复全部类型、全部配置状态和空搜索词。"], ["系统提示词为空时不得显示为正常。", "无客户专属提示词时客户仍在列表且启用绑定数为 0。", "停用模板不进入当前配置；历史查看本期不做。", "客户关联多个群聊时不得暗示唯一关系。"]),
