...
| 采购入库单状态 | 入库暂存 | 操作员已暂存，暂不提交观麦 | 可在同一采购单被其它采购入库单确认时联动入库 |
...
1. 采购入库单未关联采购单时，不能确认入库，系统提示“请先关联采购单，或选择暂存入库”。
2. 采购单已关闭时，不允许继续关联或确认入库。
3. 采购单已有入库确认后，不自动关闭，仍允许新的采购入库单继续关联，用于支持分批送货。
4. 同一采购单下存在其它“入库暂存”采购入库单时，本次确认会提示并联动这些直接关联采购入库单一起入库。
...
flowchart TD
  ...
  B --> C["暂存入库"]
  C --> C1["提示：已入库暂存，暂不提交观麦"]
  C1 --> Z["结束"]
  B --> D["确认入库"]
  D --> E{"是否已关联采购单"}
  E -- "否" --> E1["提示：请先关联采购单，或选择暂存入库"]
  E1 --> Z
  E -- "是" --> F{"关联采购单是否已关闭"}
  ...
  M --> N
  N -- "放弃确认" --> Z
  N -- "确认入库" --> O["当前采购入库单 + 直接关联的入库暂存采购入库单确认入库"]
  O --> Z
```
